...
def solveSE(A):
    ...
    for i in range(size):
        ...
        if A[-(i + 1)][-2 - i] == 0b00000000:  # 解が一意に定まらないため、誤り訂正失敗
            sys.stderr.write(
                u'error: 誤り訂正に失敗しました。--no-correctionオプションの仕様を検討してください\n')
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--error-correction',
                        help=u'エラー訂正レベル(1:L 0:M 3:Q 2:H)')
    parser.add_argument('-m', '--mask', help=u'マスクパターン(0〜7)')
    parser.add_argument('-n', '--no-correction',
                        action='store_true', help=u'データブロックの誤り訂正をしない')
    parser.add_argument('-v', '--verbose',
                        action='store_true', help=u'詳細な情報を表示')
    parser.add_argument('FILE', nargs='?', default='-',
                        help=u'入力ファイル(デフォルトは標準入力)')
    ...
    if 7 <= version:
        ...
        index = -1
        for i in range(32):
            d = hammingDistance(version_information, '{0:018b}'.format(
                version_information_table[i]))
        ...
        if index == -1:  # 誤り訂正失敗
            ...
            if args.verbose:
                stdoutColor('[ OK ]', 'green')
                print(u' 誤り訂正')
            version_information = '{0:018b}'.format(
                version_information_table[index])
            if args.verbose:
    ...
    composed_unmask = ''.join(['0' if x == y else '1' if x in '01' and y in '01' else '?' for (
        x, y) in zip(composed, format_mask)])
    if args.verbose:
    ...
    if index == -1:  # 誤り訂正失敗
        if args.verbose:
            ...
            print(u' 形式情報の誤り訂正')
        # データ部が全て不明で、引数指定もなし
        if '?' in composed_unmask[:5] and not (args.error_correction != None and args.mask != None):
            sys.stderr.write(u'error: 形式情報のデータ部が不明、かつ誤り訂正に失敗しました\n')
            sys.exit(1)
        # エラー訂正レベルが不明で、引数指定もなし
        elif '?' in composed_unmask[:2] and args.error_correction == None:
            sys.stderr.write(u'error: 形式情報のデータ部が不明、かつ誤り訂正に失敗しました\n')
            sys.exit(1)
        # マスクパターンが不明で、引数指定もなし
        elif '?' in composed_unmask[2:5] and args.mask == None:
            sys.stderr.write(u'error: 形式情報のデータ部が不明、かつ誤り訂正に失敗しました\n')
            sys.exit(1)
    ...
    for i in range(block_num):
        ...
        if offset % block_num != 0:  # 端数のある場合
            ...
            if (block_num - remain) <= i:
                t.append(blocks[(offset // block_num) *
                                block_num + (i - (block_num - remain))])
            ...
            for i in range((length + 1) // 2):
                if i == (length + 1) // 2 - 1:  # 最後の1または2けた
                    if length % 2 == 0:
                        ...
                        data.extend(
                            [ord(alphanumeric_table[num // 45]), ord(alphanumeric_table[num % 45])])
                    else:
                    ...
                    num = int(data_bits[:11], 2)
                    data_bits = data_bits[11:]
                    data.extend([ord(alphanumeric_table[num // 45]),
                                 ord(alphanumeric_table[num % 45])])
            if args.verbose:
                print u'デコードされたデータ: {0}'.format(repr(map(hex, data)))
        # 8ビットバイトのとき
        if mode == 0b0100:
